...
- Providers in scope: OpenAI Responses, OpenAI Codex Responses, Foundry, Anthropic Messages, faux.
...
- `epsilon.llm.stream` unified `stream` / `complete`
...
- `OPENAI_TOOL_CALL_PROVIDERS` and the Foundry OpenAI-compatible transport accept the upstream allowed-provider sets
- plain `StreamOptions(...)` are normalized inside each provider stream function; xhigh gating still uses `supports_xhigh(model)` for OpenAI-compatible responses providers
